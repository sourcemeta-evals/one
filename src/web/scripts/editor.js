...
const setHighlightData = StateEffect.define();
...
const highlightDataPlugin = StateField.define({
  create() { return []; },
  update(data, transaction) {
    for (const effect of transaction.effects) {
      if (effect.is(setHighlightData)) {
        return effect.value;
      }
    }
...
export class Editor {
  ...
  highlight(range, color) {
    ...
    if (lineStart === lineEnd && columnStart > columnEnd + 1) {
      throw new RangeError(
        `columnStart ${columnStart} cannot be greater than columnEnd + 1 (${columnEnd + 1}) on the same line`
      );
    }
    ...
    const currentData = this.view.state.field(highlightDataPlugin);
    const newData = [...currentData, { range: [...range], color }];

    this.view.dispatch({
      effects: [
        setHighlights.of(newSet),
        setHighlightData.of(newData)
      ]
    });
  }

  highlights() {
    return this.view.state.field(highlightDataPlugin);
  }
