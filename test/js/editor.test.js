...
describe("Editor highlights", () => {
  ...
  test("highlight preserves the color value", () => {
    const editor = new Editor(container, "hello world");
  ...
  test("highlight partial line from middle", () => {
    const editor = new Editor(container, "hello world");
  ...
  test("highlight spanning two lines", () => {
    const editor = new Editor(container, "first\nsecond");
    editor.highlight([ 1, 3, 2, 4 ], "#aabbcc");
    const result = editor.highlights();
    assert.deepStrictEqual(result[0].range, [ 1, 3, 2, 4 ]);
    assert.strictEqual(result[0].color, "#aabbcc");
  });

  test("highlight on second line only", () => {
    const editor = new Editor(container, "first\nsecond\nthird");
    editor.highlight([ 2, 1, 2, 6 ], "#112233");
    const result = editor.highlights();
  ...
  test("highlight on last line only", () => {
    const editor = new Editor(container, "first\nsecond\nthird");
    editor.highlight([ 3, 1, 3, 5 ], "#445566");
    const result = editor.highlights();
  ...
  test("three highlights with different colors", () => {
  ...
  test("highlights returns correct range after setContent", () => {
  ...
  test("highlights on content with varying line lengths", () => {
    const editor = new Editor(container, "short\na longer line here\nx");
    editor.highlight([ 2, 3, 2, 8 ], "#ff0000");
    const result = editor.highlights();
    assert.deepStrictEqual(result[0].range, [ 2, 3, 2, 8 ]);
  });
});
...
describe("Editor highlight range errors", () => {
  ...
  test("throws RangeError when start position exceeds end position", () => {
    const editor = new Editor(container, "hello world");
    assert.throws(() => {
  ...
  test("throws RangeError for lineStart beyond multiline document", () => {
  ...
  test("throws RangeError for lineEnd beyond multiline document", () => {
    const editor = new Editor(container, "a\nb\nc");
    assert.throws(() => {
  ...
  test("throws RangeError for columnStart beyond short line", () => {
  ...
  test("throws RangeError for columnEnd beyond short line", () => {
    const editor = new Editor(container, "ab");
    assert.throws(() => {
      editor.highlight([ 1, 1, 1, 3 ], "#ff0000");
    }, RangeError);
  ...
  test("does not throw for valid multiline highlight", () => {
    const editor = new Editor(container, "hello\nworld");
    assert.doesNotThrow(() => {
  ...
  test("does not throw for highlight at last line and column", () => {
  ...
  test("valid highlight still works after a RangeError", () => {
    const editor = new Editor(container, "hello");
    assert.throws(() => {
      editor.highlight([ 0, 1, 1, 5 ], "#ff0000");
    }, RangeError);
